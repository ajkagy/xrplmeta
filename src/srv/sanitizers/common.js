...
export function sanitizePoint({ defaultToLatest = false } = {}){
	return ({ ctx, ...args }) => {
		...
		if(args.hasOwnProperty('sequence')){
			...
			if(defaultToLatest){
				sequence = available.sequence.end
				time = available.time.end
			}else throw {
				type: `missingParam`,
	...
	return ({ ctx, prefer_sources, ...args }) => {
		if(prefer_sources){
			...
			for(let source of prefer_sources){
				...
				if(ctx.config.trustlist){
					if(
						ctx.config.trustlist.some(
							list => list.id === source
						)
					)
						continue
